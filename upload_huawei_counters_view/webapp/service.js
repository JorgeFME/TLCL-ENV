...
async function nodeBulkService() {
    ...
    postSiteHuaweiCounters(arrayApi)
    // if (fileType == 'ERICSSON') {
    //     await postSiteEricsson(arrayApi);
    // } else if (fileType == 'HUAWEI') {
    //     await postSiteHuaweiCounters(arrayApi);
    // }
}


async function postSiteHuaweiCounters(data) {
    try {
        ...
        do {
            response = await axios.post('https://demos-ee-multitenant-historicback.cfapps.us10.hana.ondemand.com/HuaweiCounters', data);
...
async function activateHuaweiCountersFlow() {
    try {
        const response = await axios.post('https://global-hitss-app-dataintelligenceapi-srv.cfapps.us10.hana.ondemand.com/apiActivateFlow', {
            apiFlowName: "Carga Counters Huawei V2"
        });
